...
### Fixed
- **HA Repair issue `charge_target_unreachable` could persist after the algorithm recovered** (C-022 / C-026). Observed live 2026-05-18: during an active smart-charge session the algorithm raised `charge_target_unreachable` (correctly), then solar improved and the D-043 ahead-of-schedule re-defer fired. From that point on the listener's `_adjust_charge_power` exited via the pre-start defer or D-043 re-defer early returns on every tick, never reaching the post-adjust clear at the bottom of the function — so the Repair issue stayed up while `sensor.foxess_smart_operations` simultaneously reported `charge_target_reachable: true`. Sensor and Repair contradicted each other for the rest of the session, violating the C-026 invariant that persistent errors must reflect actual state. Fix: a closure-local `_maybe_clear_unreachable_on_early_return()` helper re-runs `is_charge_target_reachable` (same parameter list as the post-adjust call, preserving C-038 sensor-listener parity) before each early return; when the algorithm now reports the target reachable, the helper clears the Repair-registry entry and the in-state flag. Three-test regression suite (`tests/test_smart_charge_unreachable_clear.py::TestUnreachableIssueClearOnEarlyReturn`) covers the D-043 re-defer path, the pre-start defer path, and an inverse case proving the helper does not over-eagerly call the registry when no issue was raised. Brand-agnostic per C-040 — uses `smart_battery.testing.FakeAdapter`.
- **`_tight_window` test helper could yield a same-day window straddling midnight** (C-031 / C-009). When `now` was within ~2 minutes of 23:59 the helper computed a window with `start = now` and `end = now + minutes`, which crossed midnight and was invalid per the FoxESS API contract. CI failures concentrated in the 23:50–23:59 UTC band (run 26006442121, 2026-05-17): the session registered with `end ≈ now + 0..2 seconds`, `async_track_point_in_time` fired `_on_timer_expire` ~1 s after the service call, `cancel_smart_discharge` cleared `dd.smart_discharge_state`, and the sensor reverted to `idle` before the test could observe `discharging`. Fix: `_tight_window` now sleeps until 00:00:05 of the next UTC day when no fittable same-day window exists. Worst-case sleep ≈ 62 s (well within the 15-minute job timeout). New `test_window_at_2358_sleeps_past_midnight` uses a fake clock that tracks the helper's mocked `time.sleep` calls to verify post-midnight behaviour without actually sleeping in CI.
