...
### Changed
- **Solar box is now hidden (not swapped to Gen Load) when solar has not been detected** (supersedes the Gen Load swap shipped in beta.1 / beta.2). While `solar_seen=False`, the overview card omits the solar box entirely — the responsive grid reflows to a 3-box layout (House / Grid / Battery) per D-036. Motivation: the Gen Load swap read `loadsPower`, which is exactly what the House box already shows — two boxes with the same number, differently labelled, in the default layout. Hiding the slot is more honest and the card's CSS already handles 1/3/4 box layouts. Users who want to repurpose the slot (e.g. for a generator power sensor) can list the `solar` box in `boxes:` with their own `label` and `icon`; any explicit user config overrides the auto-hide. The `solar_seen` flag semantics from beta.2 are unchanged (50 W threshold + 20 min timeout); only the card's response to `solar_seen=False` changes. The now-unused `gen_load` i18n key is removed from all 10 locales.

## 1.0.15-beta.2

### Changed
- **Solar-seen flag now reverts after 20 minutes of no above-threshold readings** (supersedes the sticky-for-process-lifetime semantics shipped in beta.1). The `solar_seen` flag is now `True` iff a `pvPower` reading strictly above `SOLAR_SEEN_THRESHOLD_KW` (default 0.05 kW / 50 W) has been observed within the last `SOLAR_SEEN_TIMEOUT_MIN` minutes (default 20). Zero / sub-threshold / `None` / negative / missing readings do NOT refresh the window — otherwise the timeout would never fire on installs that saw a transient positive reading and then went quiet. Motivation: on AC-coupled or unwired-PV installs that happen to see a single transient positive daytime reading, the original sticky design would claim "solar" for the rest of the process — opposite of what the Gen Load fallback is supposed to communicate. The 20-minute window absorbs brief cloud dips and ~5 s WS flicker; the timeout keeps the display honest overnight and on sites that never really had solar. Four new coordinator test cases cover the timeout revert, re-flip after a new positive reading, window-non-refresh on zero readings, and the sub-threshold / at-threshold / above-threshold boundary; the previous `test_flag_is_sticky_across_zero_readings` is rewritten as `test_flag_is_sticky_within_timeout_window`. Twenty tests total (14 coordinator + 3 sensor-attribute + 3 Playwright card). **Note**: beta.3 replaces the Gen Load swap with a hide-by-default — see the beta.3 entry above.

## 1.0.15-beta.1

### Added
- **"No solar yet seen" detection with Gen Load fallback on the overview card**: the `FoxESSDataCoordinator` tracks a process-lifetime `solar_seen` flag that starts `False` on every restart and flips to `True` the first time a `pvPower > 0` observation arrives (from either the REST poll or the WS injection path). While the flag is `False`, the overview card renders the solar box with label "Gen Load", the `loadsPower` value, and a ⚡ icon in place of the usual sun; once the flag flips, the card reverts to normal Solar rendering. Target users: AC-coupled inverters, battery-only installs, or hybrid installs where the PV strings are genuinely unwired. User-supplied `box.label` / `box.icon` overrides still win; a missing attribute defaults to `True` so existing installs see no change. **Note**: beta.1 shipped with a sticky-for-process-lifetime rule; beta.2 replaces it with a 20-minute timeout + 50 W threshold — see the beta.2 entry above for the final shipping semantics.
