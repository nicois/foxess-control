...
### Added
- **"No solar yet seen" detection with Gen Load fallback on the overview card**: the `FoxESSDataCoordinator` now tracks a runtime `solar_seen` flag that starts `False` on every restart, flips to `True` any time a `pvPower` reading above 50 W (`SOLAR_SEEN_THRESHOLD_KW = 0.05`) arrives (from either the REST poll or the WS injection path), and reverts to `False` again if no such reading has been observed for 20 minutes (`SOLAR_SEEN_TIMEOUT_MIN = 20`). The window absorbs brief cloud dips and WS flicker without flapping the display; the timeout keeps the display honest overnight and on AC-coupled / battery-only sites where a transient positive reading may have landed during the day but no real solar ever followed. Zero / `None` / negative / sub-threshold readings do NOT refresh the window — only a strictly-above-threshold reading resets the timeout. The flag is not persisted across restarts. Surfaced as an `extra_state_attribute` on the `pv_power` sensor only (no attribute pollution on unrelated sensors, no new HA entity). The overview card reads the attribute and, while the flag is `False`, renders the solar box with label "Gen Load", the `loadsPower` value, and a ⚡ icon in place of the usual sun icon; once the flag flips, the card reverts to normal Solar rendering. Target users: AC-coupled inverters, battery-only installs, or hybrid installs where the PV strings are genuinely unwired — replacing a permanently-stuck `0.0 kW` solar reading with the useful house-load value. User-supplied `box.label` / `box.icon` overrides still win (backwards-compatible); a missing attribute defaults to `True` so existing installs see no change. Twenty-test regression suite (`tests/test_solar_seen.py`): 14 coordinator lifecycle cases (fresh=False, REST/WS both flip on positive, both keep False on zero, sticky within the 20-minute window, reverts after timeout, re-flips on a subsequent positive reading, zero readings do not refresh the timestamp, sub-threshold / at-threshold / above-threshold / None / negative / missing cases), 3 sensor-attribute cases (attribute appears on `pv_power` only, reflects flag state), 3 Playwright card cases (gen-load rendering at `solar_seen=False`, normal solar rendering when `True`, sticky `True` with zero `pvPower` still shows Solar). Traces C-020 (operational transparency — replace stuck-zero noise with actionable state) and C-026.
