...
### Added
- **"No solar yet seen" detection with Gen Load fallback on the overview card**: the `FoxESSDataCoordinator` now tracks a process-lifetime `solar_seen` flag that starts `False` on every restart and flips to `True` the first time a `pvPower > 0` observation arrives (from either the REST poll or the WS injection path). Zero / `None` / negative / missing readings are ignored defensively, and the flag is sticky — a later zero-solar reading does not revert it. Surfaced as an `extra_state_attribute` on the `pv_power` sensor only (no attribute pollution on unrelated sensors, no new HA entity, no persistence across restarts per spec). The overview card reads the attribute and, while the flag is `False`, renders the solar box with label "Gen Load", the `loadsPower` value, and a ⚡ icon in place of the usual sun icon; once the flag flips, the card reverts to normal Solar rendering. Target users: AC-coupled inverters, battery-only installs, or hybrid installs where the PV strings are genuinely unwired — replacing a permanently-stuck `0.0 kW` solar reading with the useful house-load value. User-supplied `box.label` / `box.icon` overrides still win (backwards-compatible); a missing attribute defaults to `True` so existing installs see no change. Sixteen-test regression suite (`tests/test_solar_seen.py`): 10 coordinator lifecycle cases (fresh=False, REST/WS both flip on positive, both keep False on zero, sticky across multiple zeros, tiny 0.001 kW counts, None/negative/missing are defensive no-ops), 3 sensor-attribute cases (attribute appears on `pv_power` only, reflects flag state), 3 Playwright card cases (gen-load rendering at `solar_seen=False`, normal solar rendering when `True`, sticky `True` with zero `pvPower` still shows Solar). Traces C-020 (operational transparency — replace stuck-zero noise with actionable state) and C-026.
