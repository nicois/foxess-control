...
from .foxess_adapter import (
    FoxESSCloudAdapter,
    FoxESSEntityAdapter,
    _build_override_group,  # noqa: F401 — re-exported for tests
    _check_schedule_safe,  # noqa: F401 — re-exported for tests
    _groups_overlap,  # noqa: F401 — re-exported for tests
    _is_expired,  # noqa: F401 — re-exported for tests
    _is_placeholder,
    _merge_with_existing,  # noqa: F401 — re-exported for tests
    _remove_mode_from_schedule,
    _sanitize_group,  # noqa: F401 — re-exported for tests
...
def _build_foxess_adapter(
    ...
    adapter = FoxESSCloudAdapter(
        ...
        api_min_soc=state.get("api_min_soc", _cfg(hass).api_min_soc),
        start=state["start"],
        end=state["end"],
    ...
    if charge_data is not None:
        try:
            ...
            _LOGGER.warning(
                ...
                "(backend=%s, data=%s): %s",
                "entity" if _cfg(hass).entity_mode else "cloud",
                charge_data,
                exc,
            )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    from .domain_data import build_config

    dd.config = build_config(
        dict(entry.options),
        inverter_max_power_w=inverter.max_power_w if inverter else None,
    )
        ...
        if dd.battery_compound_id is None:
            ...
            async def _discover_battery_id() -> None:
                ...
                if _plant_id is None and inverter is not None:
                    try:
                        _plant_id = await hass.async_add_executor_job(
                            inverter.get_plant_id
                        )
                        dd_inner.plant_id = _plant_id
                    except Exception:
    ...
    if _cfg(hass).ws_mode == WS_MODE_ALWAYS:
        await _maybe_start_realtime_ws(hass)
...
from ._services import _register_services as _register_services  # noqa: E402
