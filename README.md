...
| Additional solar power variable | _(blank)_ | FoxESS variable name | **Cloud mode, optional.** For AC-coupled systems where a second inverter's generation is reported in a separate FoxESS telemetry variable (commonly `meterPower2`) rather than the FoxESS `pvPower` reading. When set, that variable is polled and its value is **added** to `pvPower`, so the smart charge/discharge algorithms see true total generation. Added raw (sign depends on your CT orientation); leave blank unless you have a second AC-coupled inverter. If the named variable is never returned by the API (typo or unsupported name), a diagnostics error is recorded after a few polls. |
