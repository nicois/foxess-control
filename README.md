...
| Smart Charge Headroom | 10% | 0-25% | Spare capacity reserved during `smart_charge` for transient load spikes. Applied as both a time buffer (plan to finish in 90% of the window) and a power multiplier (request 110% of the calculated rate). Lower values charge more slowly and defer longer; higher values start earlier and charge faster. Set to 0 for no headroom (not recommended — transient loads may prevent reaching the target). |
