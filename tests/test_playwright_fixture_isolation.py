...
def test_pytest_playwright_does_not_leak_running_loop(tmp_path: Path) -> None:
    ...
    # project's pytest config (which pins -n auto for pytest-xdist)
    # is NOT inherited.
...
# Guard against the sandbox helper silently breaking: the shutil import
# is used only if a future maintainer extends this test to copy files
# into the sandbox.  Referenced here to avoid an unused-import lint.
_ = shutil
