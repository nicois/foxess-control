...
| C-016 Cancel listeners before awaits | D-018 | -- | PARTIAL |
...
| C-020 Operational transparency | D-008, D-009, D-021, D-027 | `TestDataSourceTracking` (3), `TestFoxESSPolledSensor::test_data_source_*` (2), `TestSessionContextFilter` (7), `TestInstallRemove` (2), `TestDebugLogHandlerWithSession` (3) | COVERED |
| C-021 Brand-agnostic code in common package | D-022 | `test_vendored_copy_matches_canonical` (indirect) | PARTIAL |
| C-022 Unreachable charge target surfaced | -- | `TestIsChargeTargetReachable` (7) | PARTIAL |
...
| C-026 Proactive error surfacing | -- | `TestErrorSurfacing` (2) | PARTIAL |
...
| C-029 E2E for HA-dependent behaviour | -- | `e2e/test_e2e.py` (28), `e2e/test_ui.py` (38) | ACCEPTED |
...
### Actionable (PARTIAL)
- **C-016**: Cancel-before-await ordering — has D-018 but no test. The
  race-prevention pattern in `__init__.py` is structural and not
  verified by any test. A test would need to simulate concurrent timer
  firing during cancellation.
- **C-021**: Brand-agnostic code — D-022 traces to it but only
  addresses entity mode, not the broader code-separation principle.
  No dedicated test for the package boundary invariant.
- **C-022**: Unreachable charge target — algorithm exists and is tested
  (`TestIsChargeTargetReachable`) but no D-NNN design decision
  documents the detection/notification approach.
- **C-026**: Proactive error surfacing — tested (`TestErrorSurfacing`)
  but no D-NNN documents the error surfacing mechanism.
...
- **C-028**: Simulator over mocks — testing methodology enforced by
  infrastructure (simulator exists, tests use it). No D-NNN expected.
...
- **Total constraints**: 31 (excluding C-023 proposed)
- **Fully covered**: 20 (65%)
- **Partial (actionable)**: 4 (13%) — C-016, C-021, C-022, C-026
- **Accepted (non-actionable)**: 7 (22%) — C-013, C-015, C-028–C-032
- **Gaps**: 0
