...
|---|---|---|---|
| C-001 No grid import during discharge | D-001, D-002, D-003, D-004, D-005, D-007, D-037 | `TestCalculateDischargePower` (21), `TestShouldSuspendDischarge` (21), `TestDischargePowerFeedinConstraint` (10), `TestCalculateDischargeDeferredStart` (13) | COVERED |
...
| C-003 Session identity prevents races | D-017, D-018 | `TestSessionPersistence` (3), `TestRecoverSessions` (13) | COVERED |
...
| C-020 Operational transparency | D-008, D-009, D-021, D-027, D-030, D-033, D-035, D-036, D-038, D-039, D-040 | `TestDataSourceTracking` (3), `TestFoxESSPolledSensor::test_data_source_*` (2), `TestSessionContextFilter` (7), `TestInstallRemove` (2), `TestDebugLogHandlerWithSession` (3), `test_data_source_badge_matches_mode` (staleness suffix), `test_stale_badge_shown_for_old_api_data` (6), `TestBMSBatteryTemperature` (6), `TestBatteryDetailEndpoint` (3), `TestCompoundIdFromWebSocket` (3), `TestOverviewCard::test_node_click_opens_more_info`, `TestOverviewCard::test_default_config_renders_all_four_boxes`, `TestOverviewCard::test_custom_boxes_*` (3), `TestFormInputPersistence` (5) | COVERED |
...
| C-024 Safe state on failure | D-023, D-025, D-026, D-031, D-032, D-034 | `TestCallbackExceptionSafety` (2), `TestTransientApiErrorResilience` (3), `TestStaleWorkModeAfterCleanupFailure` (2), `TestCircuitBreakerCharge` (4), `TestCircuitBreakerDischarge` (2), `test_returns_inverter`, `test_raises_when_no_entries`, `TestRecoverSessions` (13), `TestReloadRecovery` (14 E2E) | COVERED |
| C-025 Session boundary cleanliness | D-026, D-032 | `TestSessionBoundaryCleanness` (2), `TestStaleWorkModeAfterCleanupFailure` (2), `TestRecoverSessions` (13), `TestReloadRecovery` (14 E2E) | COVERED |
...
| C-029 E2E for HA-dependent behaviour | -- | `tests/e2e/test_e2e.py` (46), `tests/e2e/test_ui.py` (44) = 128 E2E (with parametrization) | ACCEPTED |
...
- **C-029**: E2E for HA-dependent behaviour — enforced by E2E test
  suite existence (88 tests). No D-NNN expected.
...
- **D-020**: start_soc persistence for progress display — traces to
  C-020 (operational transparency) via progress bar accuracy after
  restart. Reclassified from UNJUSTIFIED to implicit C-020 trace.

### Bidirectional trace mismatches
(none — D-016 ↔ C-018 mismatch fixed 2026-04-19)
...
- **Orphan tests**: ~60 unit (test_services.py partially mapped, test_sensor.py display tests unmapped)
- **Unit tests**: 670
- **E2E tests**: 128 (46 `test_e2e.py` + 44 `test_ui.py`, parametrized across cloud/entity)
...
- action-exceptions, config-entry-unloading, docs-configuration-parameters,
  ...
  test-coverage (670 unit + 128 E2E)
